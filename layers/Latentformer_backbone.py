...
class Latentformer_backbone(nn.Module):
    def __init__(self, configs, c_in:int, context_window:int, target_window:int, patch_len:int, stride:int, max_seq_len:Optional[int]=1024, 
        ...
        # Backbone 
        self.backbone = TSTiEncoder(patch_len=patch_len, max_seq_len=max_seq_len,
                                n_layers=n_layers, d_model=d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff,
                                attn_dropout=attn_dropout, dropout=dropout, act=act, key_padding_mask=key_padding_mask, padding_var=padding_var,
                                attn_mask=attn_mask, res_attention=res_attention, pre_norm=pre_norm, store_attn=store_attn,
                                pe=pe, learn_pe=learn_pe, verbose=verbose, **kwargs)
        # Head  
        self.predict = MLP(configs.num_layers, d_model*context_window, [x*context_window for x in configs.hidden_size], target_window)
    ...
    def forward(self, z):                                                                   # [b, c, l]
        ...
        z = self.backbone(z)                                                                # [b, c, l/2, d]   
        z = torch.fft.irfft2(z, s=(C, L), dim=(1, 2), norm="ortho")                      
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    def __init__(self, patch_len, max_seq_len=1024,
    ...
    def forward(self, x) -> Tensor:                                              # [b, c, l/2]
        b,c,t = x.size()
        x = x.unsqueeze(-1) * self.embeddings                                    # [b, c, l/2, d]
        o1_real = x.real.reshape(b*c*t,-1)                                       # [b*c*l/2, d]
        o1_imag = x.imag.reshape(b*c*t,-1)
        y = torch.stack([o1_real, o1_imag], dim=-2)                              # [b*c*l/2, 2, d]   
        y = self.encoder(y).permute(0,2,1)                                       # [b*c*l/2, d, 2]        
        y = torch.view_as_complex(y.contiguous()).view(b,c,t,-1)                 # [b, c, l/2, d]   
        return y   
